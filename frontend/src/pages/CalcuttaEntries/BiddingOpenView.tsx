...
export function BiddingOpenView({
  ...
  const entryStatusLabel = !currentUserEntry ? 'Not Started' : (statusLabelMap[entryStatus] ?? entryStatus);
  const entryStatusVariant = !currentUserEntry ? 'secondary' : (statusVariantMap[entryStatus] ?? 'secondary');
  ...
  return (
    <PageContainer>
      ...
      {!currentUserEntry ? (
        ...
        <Link to={`/calcuttas/${calcuttaId}/entries/${currentUserEntry.id}`} className="block">
          <Card variant="accent" padding="compact" className="hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h3 className="text-lg font-semibold text-foreground">{currentUserEntry.name}</h3>
                <Badge variant={entryStatusVariant as 'secondary' | 'success' | 'warning'}>{entryStatusLabel}</Badge>
              </div>
              <svg
      ...
      <div className="mt-6 p-6 border border-blue-200 rounded-lg bg-primary/10 text-center">
        ...
        <p className="text-lg font-semibold text-blue-900 mb-2">Tournament hasn't started yet</p>
        <p className="text-primary">
          Come back once the tournament starts for the full leaderboard, ownership breakdowns, and live scoring.
        </p>
        {tournamentStartingAt && (
          <p className="mt-3 text-sm text-primary">Portfolios revealed {formatDate(tournamentStartingAt, true)}</p>
        )}
      </div>
