...
export function PayoutsForm({ poolId, onSuccess }: PayoutsFormProps) {
  const queryClient = useQueryClient();

  const payoutsQuery = useQuery({
    queryKey: queryKeys.pools.payouts(poolId),
    enabled: Boolean(poolId),
    queryFn: () => poolService.getPayouts(poolId),
  });
  ...
  const payoutMutation = useMutation({
    mutationFn: (payouts: PayoutRow[]) => {
      return poolService.replacePayouts(poolId, payouts);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.pools.payouts(poolId) });
      onSuccess();
    },
  });
