...
import { bestTeamsColumns, bestInvestmentsColumns, bestEntriesColumns, bestCareersColumns } from './HallOfFame/columns';
...
export function HallOfFamePage() {
  ...
  return (
    <PageContainer>
      <Breadcrumb items={[{ label: 'Admin', href: '/admin' }, { label: 'Hall of Fame' }]} />
      <PageHeader
        title="Hall of Fame"
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="bestTeams">
          <Card>
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Which teams performed best relative to expectations? Performance adjusted for inflation, because a 12-seed
              making the Sweet 16 is more impressive than a 1-seed doing it.
            </p>
            ...
            {bestTeamsQuery.isError && (
              <ErrorState
                error={
                  bestTeamsQuery.error instanceof Error ? bestTeamsQuery.error.message : 'Failed to load best teams'
                }
        ...
        <TabsContent value="bestInvestments">
          <Card>
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Which individual picks returned the most? Returns adjusted for inflation so every pool is on equal
              footing.
            </p>
            ...
            {bestInvestmentsQuery.isError && (
              <ErrorState
                error={
                  bestInvestmentsQuery.error instanceof Error
                    ? bestInvestmentsQuery.error.message
                    : 'Failed to load best investments'
                }
        ...
        <TabsContent value="bestEntries">
          <Card>
            ...
            {bestEntriesQuery.isError && (
              <ErrorState
                error={
                  bestEntriesQuery.error instanceof Error
                    ? bestEntriesQuery.error.message
                    : 'Failed to load best entries'
                }
          ...
          <Card>
            ...
            <div className="mb-4 flex items-center gap-2">
              <input
                ...
                className="h-4 w-4"
              />
              <label htmlFor="hideInactiveCareers" className="text-sm text-foreground">
            ...
            {bestCareersQuery.isError && (
              <ErrorState
                error={
                  bestCareersQuery.error instanceof Error
                    ? bestCareersQuery.error.message
                    : 'Failed to load best careers'
                }
            ...
            {bestCareersQuery.data && (
              <DataTable columns={bestCareersColumns} data={filteredCareers} pagination pageSize={25} />
            )}
          </Card>
        </TabsContent>
      </Tabs>
