...
export function RaceTab({ portfolios, dashboard }: RaceTabProps) {
  ...
  const data = useMemo(
    () => buildBumpChartData(portfolios, dashboard.roundStandings, sortMode),
    [portfolios, dashboard.roundStandings, sortMode],
  );

  const colorMap = useMemo(() => getEntryColorById(portfolios), [portfolios]);
  const nameToColor = useMemo(() => {
    const m = new Map<string, string>();
    for (const portfolio of portfolios) {
      m.set(portfolio.name, colorMap.get(portfolio.id) ?? '#6B7280');
    }
    return m;
  }, [portfolios, colorMap]);
  ...
  const chartHeight = Math.max(400, portfolios.length * 30);
