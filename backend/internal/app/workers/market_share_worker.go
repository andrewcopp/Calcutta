...
type MarketShareWorker struct {
	pool     *pgxpool.Pool
	progress ProgressWriter
}

func NewMarketShareWorker(pool *pgxpool.Pool, progress ProgressWriter) *MarketShareWorker {
	if progress == nil {
		progress = NewDBProgressWriter(pool)
	}
	return &MarketShareWorker{pool: pool, progress: progress}
}
...
func (w *MarketShareWorker) Run(ctx context.Context) {
	w.RunWithOptions(ctx, defaultMarketShareWorkerPollInterval, defaultMarketShareWorkerStaleAfter)
}

func (w *MarketShareWorker) RunWithOptions(ctx context.Context, pollInterval time.Duration, staleAfter time.Duration) {
	if w == nil || w.pool == nil {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-t.C:
			job, ok, err := w.claimNextMarketShareJob(ctx, workerID, staleAfter)
			if err != nil {
			...
			_ = w.processMarketShareJob(ctx, workerID, job)
		}
	}
}

func (w *MarketShareWorker) claimNextMarketShareJob(ctx context.Context, workerID string, staleAfter time.Duration) (*marketShareJob, bool, error) {
	if ctx == nil {
...
func (w *MarketShareWorker) processMarketShareJob(ctx context.Context, workerID string, job *marketShareJob) bool {
	if job == nil {
		return false
	}

	w.updateRunJobProgress(ctx, "market_share", job.RunID, 0.05, "start", "Starting market share job")
	...
	if resolvedRunner == "" {
		...
		w.updateRunJobProgress(ctx, "market_share", job.RunID, 1.0, "failed", err.Error())
		w.failMarketShareJob(ctx, job, err)
		log.Printf("market_share_worker fail worker_id=%s run_id=%s run_key=%s err=%v", workerID, job.RunID, job.RunKey, err)
		return false
	}

	w.updateRunJobProgress(ctx, "market_share", job.RunID, 0.15, "runner_resolved", "Resolved python runner")

	log.Printf("market_share_worker start worker_id=%s run_id=%s run_key=%s", workerID, job.RunID, job.RunKey)
	w.updateRunJobProgress(ctx, "market_share", job.RunID, 0.25, "running", "Running python market share runner")
	...
	if err != nil {
		...
		w.updateRunJobProgress(ctx, "market_share", job.RunID, 1.0, "failed", msg)
		w.failMarketShareJob(ctx, job, errors.New(msg))
	...
	w.updateRunJobProgress(ctx, "market_share", job.RunID, 1.0, "succeeded", "Completed")
...
func (w *MarketShareWorker) failMarketShareJob(ctx context.Context, job *marketShareJob, err error) {
	msg := "unknown error"
	if err != nil {
		msg = err.Error()
	}
...
func (w *MarketShareWorker) updateRunJobProgress(ctx context.Context, runKind string, runID string, percent float64, phase string, message string) {
