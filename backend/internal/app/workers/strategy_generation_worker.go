...
type StrategyGenerationWorker struct {
	pool     *pgxpool.Pool
	progress ProgressWriter
}

func NewStrategyGenerationWorker(pool *pgxpool.Pool, progress ProgressWriter) *StrategyGenerationWorker {
	if progress == nil {
		progress = NewDBProgressWriter(pool)
	}
	return &StrategyGenerationWorker{pool: pool, progress: progress}
}
...
func (w *StrategyGenerationWorker) Run(ctx context.Context) {
	w.RunWithOptions(ctx, defaultStrategyGenWorkerPollInterval, defaultStrategyGenWorkerStaleAfter)
}

func (w *StrategyGenerationWorker) RunWithOptions(ctx context.Context, pollInterval time.Duration, staleAfter time.Duration) {
	if w == nil || w.pool == nil {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-t.C:
			job, ok, err := w.claimNextStrategyGenerationJob(ctx, workerID, staleAfter)
			if err != nil {
			...
			_ = w.processStrategyGenerationJob(ctx, workerID, job)
		}
	}
}

func (w *StrategyGenerationWorker) claimNextStrategyGenerationJob(ctx context.Context, workerID string, staleAfter time.Duration) (*strategyGenJob, bool, error) {
	if ctx == nil {
...
func (w *StrategyGenerationWorker) processStrategyGenerationJob(ctx context.Context, workerID string, job *strategyGenJob) bool {
	if job == nil {
		return false
	}

	log.Printf("strategy_generation_worker start worker_id=%s run_id=%s run_key=%s", workerID, job.RunID, job.RunKey)
	w.updateRunJobProgress(ctx, "strategy_generation", job.RunID, 0.05, "start", "Starting strategy generation job")
	w.updateRunJobProgress(ctx, "strategy_generation", job.RunID, 0.25, "running", "Generating recommended entry bids")
...
func (w *StrategyGenerationWorker) failStrategyGenerationJob(ctx context.Context, job *strategyGenJob, err error) {
	msg := "unknown error"
	if err != nil {
		msg = err.Error()
	}
	if job != nil {
		w.updateRunJobProgress(ctx, "strategy_generation", job.RunID, 1.0, "failed", msg)
	}
...
func (w *StrategyGenerationWorker) updateRunJobProgress(ctx context.Context, runKind string, runID string, percent float64, phase string, message string) {
