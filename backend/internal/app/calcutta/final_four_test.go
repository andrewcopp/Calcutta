...
func entry(id string) *models.CalcuttaEntry {
...
func portfolio(id, entryID string) *models.CalcuttaPortfolio {
	return &models.CalcuttaPortfolio{ID: id, EntryID: entryID}
}

func portfolioTeam(portfolioID, teamID string, ownership float64) *models.CalcuttaPortfolioTeam {
	return &models.CalcuttaPortfolioTeam{
...
func payout(position, amountCents int) *models.CalcuttaPayout {
	return &models.CalcuttaPayout{Position: position, AmountCents: amountCents}
}
...
func TestThatChampionGetsExtraWinsInScoring(t *testing.T) {
	// GIVEN a bracket with 4 FF teams, one entry owns team A at 100%
	bracket := buildFinalFourBracket(
		bracketTeam("A", "sa", 1, "East"),
		bracketTeam("B", "sb", 2, "West"),
		bracketTeam("C", "sc", 1, "South"),
	...
	var championPoints, semiOnlyPoints float64
	for _, o := range result {
		if o.Champion.TeamID == "A" {
			championPoints = o.Standings[0].TotalPoints
			break
		}
	}
	for _, o := range result {
		if o.Semifinal1Winner.TeamID == "A" && o.Champion.TeamID != "A" {
			semiOnlyPoints = o.Standings[0].TotalPoints
			break
		}
	}

	// THEN champion outcome gives more points than semifinal-only outcome
	if championPoints <= semiOnlyPoints {
		t.Errorf("expected champion points (%.2f) > semifinal-only points (%.2f)", championPoints, semiOnlyPoints)
	}
}
...
func TestThatStandingsIncludePayouts(t *testing.T) {
	...
	entries := []*models.CalcuttaEntry{entry("e1"), entry("e2")}
	portfolios := []*models.CalcuttaPortfolio{portfolio("p1", "e1"), portfolio("p2", "e2")}
	pts := []*models.CalcuttaPortfolioTeam{
		portfolioTeam("p1", "A", 1.0),
		portfolioTeam("p2", "B", 1.0),
	}
	tts := []*models.TournamentTeam{
		tournamentTeam("A", 5, 1, false),
		tournamentTeam("B", 5, 1, false),
	...
	result := ComputeFinalFourOutcomes(bracket, entries, portfolios, pts, tts, rounds, payoutsSlice)
