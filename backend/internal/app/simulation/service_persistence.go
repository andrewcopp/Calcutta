...
func (s *Service) createTournamentStateSnapshotFromBracket(
	...
	if err := tx.QueryRow(ctx, `
		INSERT INTO compute.tournament_snapshots (tournament_id, source, description)
	...
	for _, t := range teams {
		...
		_, err := tx.Exec(ctx, `
			INSERT INTO compute.tournament_snapshot_teams (
...
func (s *Service) createTournamentSimulationBatch(
	...
	if err := s.pool.QueryRow(ctx, `
		INSERT INTO compute.simulated_tournaments (
			tournament_id,
			tournament_snapshot_id,
			n_sims,
			seed,
...
func (s *Service) copyInsertSimulatedTournaments(
	...
	inserted, err := conn.Conn().CopyFrom(
		ctx,
		pgx.Identifier{"compute", "simulated_teams"},
