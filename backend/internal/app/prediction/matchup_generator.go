...
// GenerateMatchupsFromCheckpoint generates matchup predictions starting from a mid-tournament
// checkpoint. Only surviving teams are considered. Rounds already resolved (< throughRound+1)
// are skipped. The first remaining round has pMatchup = 1.0 for the known matchups since
// exactly one team occupies each bracket slot.
func GenerateMatchupsFromCheckpoint(survivors []TeamInput, throughRound int, spec *simulation_game_outcomes.Spec) ([]PredictedMatchup, error) {
	if spec == nil {
		spec = &simulation_game_outcomes.Spec{Kind: "kenpom", Sigma: 10.0}
	}
	spec.Normalize()
	...
	// Initialize pAdvance = 1.0 for all survivors
