...
position_groups AS (
  SELECT
    gs.pool_id, gs.total_returns, gs.tie_size, gs.group_rank,
    1 + COALESCE(
...
payout_calc AS (
  SELECT
    e.pool_id, e.portfolio_id, e.portfolio_name, e.total_returns, e.tie_size, e.finish_position, e.tie_index,
...
enriched AS (
  SELECT
    portfolio_returns.tournament_name, portfolio_returns.tournament_year, portfolio_returns.pool_id, portfolio_returns.portfolio_id, portfolio_returns.portfolio_name, portfolio_returns.total_returns,
...
pool_enriched AS (
  SELECT
    team_investments.tournament_name, team_investments.tournament_year, team_investments.pool_id, team_investments.team_id, team_investments.school_name, team_investments.seed, team_investments.region, team_investments.team_points, team_investments.total_bid,
