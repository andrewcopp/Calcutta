...
type suiteScenarioListItem struct {
	ID                        string    `json:"id"`
...
type listSuiteScenariosResponse struct {
	Items []suiteScenarioListItem `json:"items"`
}

type createSuiteScenarioRequest struct {
	SuiteID                    string  `json:"suiteId"`
	CalcuttaID                 string  `json:"calcuttaId"`
	CalcuttaSnapshotID         *string `json:"calcuttaSnapshotId"`
	FocusStrategyGenerationRun *string `json:"focusStrategyGenerationRunId"`
...
type createSuiteScenarioResponse struct {
	ID string `json:"id"`
}

func (s *Server) registerSuiteScenarioRoutes(r *mux.Router) {
	r.HandleFunc(
		"/api/suite-scenarios",
		s.requirePermission("analytics.suite_scenarios.write", s.createSuiteScenarioHandler),
	).Methods("POST", "OPTIONS")
	r.HandleFunc(
		"/api/suite-scenarios",
		s.requirePermission("analytics.suite_scenarios.read", s.listSuiteScenariosHandler),
	).Methods("GET", "OPTIONS")
	r.HandleFunc(
		"/api/suite-scenarios/{id}",
		s.requirePermission("analytics.suite_scenarios.read", s.getSuiteScenarioHandler),
	).Methods("GET", "OPTIONS")
}

func (s *Server) listSuiteScenariosHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := s.pool.Query(r.Context(), `
		...
		LIMIT $3::int
		OFFSET $4::int
	`, nullUUIDParam(strings.TrimSpace(suiteID)), nullUUIDParam(strings.TrimSpace(calcuttaID)), limit, offset)
	if err != nil {
	...
	items := make([]suiteScenarioListItem, 0)
	for rows.Next() {
		var it suiteScenarioListItem
		if err := rows.Scan(
			&it.ID,
			&it.SuiteID,
			&it.CalcuttaID,
	...
	writeJSON(w, http.StatusOK, listSuiteScenariosResponse{Items: items})
}

func (s *Server) getSuiteScenarioHandler(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
...
func (s *Server) createSuiteScenarioHandler(w http.ResponseWriter, r *http.Request) {
	var req createSuiteScenarioRequest
	...
	focusStrategyRunID := ""
	if req.FocusStrategyGenerationRun != nil {
		v := strings.TrimSpace(*req.FocusStrategyGenerationRun)
		if v != "" {
	...
	if providedSnapshotID != "" {
		...
		created, err := createSuiteScenarioSnapshot(ctx, tx, req.CalcuttaID, excludedEntry, focusStrategyRunID, focusEntryName)
		if err != nil {
			writeErrorFromErr(w, r, err)
			return
		}
	...
	writeJSON(w, http.StatusCreated, createSuiteScenarioResponse{ID: scenarioID})
}

func createSuiteScenarioSnapshot(
	ctx context.Context,
	tx pgx.Tx,
	calcuttaID string,
	...
	if err := tx.QueryRow(ctx, `
		...
		VALUES ($1::uuid, 'suite_scenario', 'Suite scenario snapshot')
		RETURNING id
