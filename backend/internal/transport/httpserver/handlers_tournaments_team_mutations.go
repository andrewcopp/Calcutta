...
func (s *Server) createTournamentTeamHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.app.Tournament.CreateTeam(r.Context(), team); err != nil {
...
func (s *Server) updateTeamHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = s.app.Tournament.UpdateTournamentTeam(r.Context(), team)
	if err != nil {
		writeErrorFromErr(w, r, err)
		return
	}

	calcuttas, err := s.calcuttaService.GetCalcuttasByTournament(r.Context(), team.TournamentID)
	if err != nil {
	...
	school, err := s.app.School.GetByID(r.Context(), responseTeam.SchoolID)
	if err != nil {
