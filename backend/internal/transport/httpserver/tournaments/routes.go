...
func RegisterRoutes(r *mux.Router, h Handlers) {
	...
	r.HandleFunc("/api/v1/tournaments/{id}/teams", h.ListTournamentTeams).Methods("GET")
	r.HandleFunc("/api/v1/tournaments/{id}/teams", h.CreateTournamentTeam).Methods("POST")
	r.HandleFunc("/api/v1/tournaments/{id}/teams", h.ReplaceTeams).Methods("PUT")
	r.HandleFunc("/api/v1/tournaments/{tournamentId}/teams/{teamId}", h.UpdateTeam).Methods("PATCH", "OPTIONS")
	r.HandleFunc("/api/v1/tournaments/{id}/kenpom", h.UpdateKenPomStats).Methods("PUT")
	r.HandleFunc("/api/v1/tournaments/{id}/predictions", h.GetPredictions).Methods("GET")
	r.HandleFunc("/api/v1/tournaments/{id}/prediction-batches", h.ListPredictionBatches).Methods("GET")
