...
SET params_json = COALESCE(j.params_json, '{}'::jsonb) || jsonb_build_object(
    'source', COALESCE(NULLIF(j.params_json->>'source', ''), 'db_trigger')
)
WHERE j.params_json IS NULL
    OR COALESCE(NULLIF(j.params_json->>'source', ''), '') = '';
...
WHERE j.run_kind = 'calcutta_evaluation'
    AND j.run_id = cer.id
    AND (j.params_json->>'seed' IS NULL OR j.params_json->>'seed' = '');
