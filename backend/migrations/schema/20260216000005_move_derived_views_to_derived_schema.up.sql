-- Move derived portfolio views from core schema to derived schema.
-- These views join core and derived data and belong in the derived schema.

-- Drop the views from core (derived_portfolios depends on derived_portfolio_teams)
DROP VIEW IF EXISTS core.derived_portfolios;
DROP VIEW IF EXISTS core.derived_portfolio_teams;

-- Recreate derived_portfolio_teams in derived schema
...
WITH entry_bids AS (
    ...
    SELECT eb.entry_id,
        ...
        (
            ...
            END *
            CASE
                WHEN (eb.eliminated = true) THEN (eb.wins)::double precision
                ELSE (0)::double precision
            END
        ) AS actual_points,
        (
            CASE
                ...
                ELSE (0)::double precision
            END *
            (eb.wins + eb.byes + (eb.tournament_rounds - eb.wins - eb.byes))::double precision
        ) AS expected_points,
...
-- Recreate derived_portfolios in derived schema
