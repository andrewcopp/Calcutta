...
-- Medallion Architecture: Bronze (raw) -> Silver (enriched) -> Gold (business metrics)
...
CREATE TABLE IF NOT EXISTS bronze_calcuttas (
...
CREATE TABLE IF NOT EXISTS bronze_entry_bids (
    id BIGSERIAL PRIMARY KEY,
    calcutta_key VARCHAR(200) NOT NULL REFERENCES bronze_calcuttas(calcutta_key) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS bronze_payouts (
...
CREATE TABLE IF NOT EXISTS silver_predicted_game_outcomes (
    id BIGSERIAL PRIMARY KEY,
    tournament_key VARCHAR(100) NOT NULL REFERENCES bronze_tournaments(tournament_key) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS silver_predicted_market_share (
    id BIGSERIAL PRIMARY KEY,
    calcutta_key VARCHAR(200) NOT NULL REFERENCES bronze_calcuttas(calcutta_key) ON DELETE CASCADE,
    team_key VARCHAR(100) NOT NULL REFERENCES bronze_teams(team_key) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS silver_team_tournament_value (
    id BIGSERIAL PRIMARY KEY,
    tournament_key VARCHAR(100) NOT NULL REFERENCES bronze_tournaments(tournament_key) ON DELETE CASCADE,
    team_key VARCHAR(100) NOT NULL REFERENCES bronze_teams(team_key) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS gold_optimization_runs (
...
CREATE TABLE IF NOT EXISTS gold_recommended_entry_bids (
    id BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS gold_entry_simulation_outcomes (
    id BIGSERIAL PRIMARY KEY,
    run_id VARCHAR(100) NOT NULL REFERENCES gold_optimization_runs(run_id) ON DELETE CASCADE,
    entry_key VARCHAR(200) NOT NULL,
    sim_id INTEGER NOT NULL,
...
CREATE TABLE IF NOT EXISTS gold_entry_performance (
    id BIGSERIAL PRIMARY KEY,
    run_id VARCHAR(100) NOT NULL REFERENCES gold_optimization_runs(run_id) ON DELETE CASCADE,
    entry_key VARCHAR(200) NOT NULL,
...
CREATE TABLE IF NOT EXISTS gold_detailed_investment_report (
    id BIGSERIAL PRIMARY KEY,
    run_id VARCHAR(100) NOT NULL REFERENCES gold_optimization_runs(run_id) ON DELETE CASCADE,
    team_key VARCHAR(100) NOT NULL REFERENCES bronze_teams(team_key) ON DELETE CASCADE,
...
CREATE OR REPLACE VIEW view_latest_optimization_runs AS
...
CREATE OR REPLACE VIEW view_entry_rankings AS
SELECT 
    ep.run_id,
    ep.entry_key,
    ep.is_our_strategy,
...
CREATE OR REPLACE VIEW view_tournament_sim_stats AS
SELECT 
    t.tournament_key,
    t.season,
