...
CREATE TABLE core.calcuttas (
    ...
    CONSTRAINT ck_core_calcuttas_budget_positive CHECK ((budget_points > 0)),
    CONSTRAINT ck_core_calcuttas_max_bid_points_le_budget CHECK ((max_bid_points <= budget_points)),
    CONSTRAINT ck_core_calcuttas_max_bid_points_positive CHECK ((max_bid_points > 0)),
    CONSTRAINT ck_core_calcuttas_max_teams_gte_min CHECK ((max_teams >= min_teams)),
    CONSTRAINT ck_core_calcuttas_min_teams CHECK ((min_teams >= 1)),
    CONSTRAINT ck_core_calcuttas_visibility CHECK ((visibility = ANY (ARRAY['public'::text, 'unlisted'::text, 'private'::text])))
);
...
CREATE TABLE core.grants (
    ...
    deleted_at timestamp with time zone,
    granted_by uuid,
    CONSTRAINT ck_core_grants_scope_id CHECK ((((scope_type = 'global'::text) AND (scope_id IS NULL)) OR ((scope_type <> 'global'::text) AND (scope_id IS NOT NULL)))),
    CONSTRAINT ck_core_grants_scope_type CHECK ((scope_type = ANY (ARRAY['global'::text, 'calcutta'::text, 'tournament'::text]))),
    CONSTRAINT ck_core_grants_subject CHECK ((((role_id IS NOT NULL) AND (permission_id IS NULL)) OR ((role_id IS NULL) AND (permission_id IS NOT NULL))))
);
...
CREATE TABLE core.tournament_imports (
    id uuid DEFAULT public.uuid_generate_v4() NOT NULL,
    ...
    error_message text,
    CONSTRAINT ck_core_tournament_imports_status CHECK ((status = ANY (ARRAY['pending'::text, 'running'::text, 'succeeded'::text, 'failed'::text])))
);
...
CREATE TABLE core.users (
    ...
    invited_by uuid,
    CONSTRAINT ck_core_users_status CHECK ((status = ANY (ARRAY['active'::text, 'invited'::text, 'requires_password_setup'::text, 'stub'::text])))
);
...
CREATE TABLE derived.simulated_tournaments (
    id uuid DEFAULT public.uuid_generate_v4() NOT NULL,
    tournament_id uuid NOT NULL,
    tournament_snapshot_id uuid NOT NULL,
...
CREATE TABLE derived.tournament_snapshot_teams (
    id uuid DEFAULT public.uuid_generate_v4() NOT NULL,
    tournament_snapshot_id uuid NOT NULL,
    team_id uuid NOT NULL,
    wins integer NOT NULL,
...
CREATE TABLE derived.tournament_snapshots (
    id uuid DEFAULT public.uuid_generate_v4() NOT NULL,
    tournament_id uuid NOT NULL,
...
ALTER TABLE ONLY core.role_permissions ADD CONSTRAINT role_permissions_pkey PRIMARY KEY (role_id, permission_id);
ALTER TABLE ONLY core.roles ADD CONSTRAINT roles_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY core.tournament_imports ADD CONSTRAINT tournament_imports_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY derived.tournament_snapshot_teams ADD CONSTRAINT tournament_state_snapshot_teams_pkey PRIMARY KEY (id);
ALTER TABLE ONLY derived.tournament_snapshots ADD CONSTRAINT tournament_state_snapshots_pkey PRIMARY KEY (id);
...
ALTER TABLE ONLY core.permissions ADD CONSTRAINT uq_core_permissions_key UNIQUE (key);
ALTER TABLE ONLY core.roles ADD CONSTRAINT uq_core_roles_key UNIQUE (key);
ALTER TABLE ONLY core.seasons ADD CONSTRAINT uq_core_seasons_year UNIQUE (year);

ALTER TABLE ONLY derived.tournament_snapshot_teams ADD CONSTRAINT uq_analytics_tournament_state_snapshot_teams_snapshot_team UNIQUE (tournament_snapshot_id, team_id);

ALTER TABLE ONLY lab.evaluation_entry_results ADD CONSTRAINT uq_lab_evaluation_entry_results_evaluation_entry UNIQUE (evaluation_id, entry_name);
...
CREATE INDEX idx_grants_granted_by ON core.grants USING btree (granted_by);
...
CREATE INDEX idx_grants_role_id ON core.grants USING btree (role_id);
...
CREATE INDEX idx_tournament_imports_created_at ON core.tournament_imports USING btree (created_at);
CREATE INDEX idx_tournament_imports_status_created_at ON core.tournament_imports USING btree (status, created_at) WHERE (deleted_at IS NULL);
...
CREATE UNIQUE INDEX uq_grants_user_role_scope_active ON core.grants USING btree (user_id, role_id, scope_type, scope_id) WHERE ((role_id IS NOT NULL) AND (revoked_at IS NULL) AND (deleted_at IS NULL));
CREATE UNIQUE INDEX uq_tournament_imports_sha256 ON core.tournament_imports USING btree (sha256) WHERE (deleted_at IS NULL);
...
CREATE INDEX idx_derived_simulated_tournaments_snapshot_id ON derived.simulated_tournaments USING btree (tournament_snapshot_id);
CREATE INDEX idx_derived_tournament_snapshot_teams_snapshot_id ON derived.tournament_snapshot_teams USING btree (tournament_snapshot_id);
CREATE INDEX idx_derived_tournament_snapshot_teams_team_id ON derived.tournament_snapshot_teams USING btree (team_id);
CREATE INDEX idx_derived_tournament_snapshots_tournament_id ON derived.tournament_snapshots USING btree (tournament_id);
...
CREATE UNIQUE INDEX uq_analytics_tournament_simulation_batches_natural_key ON derived.simulated_tournaments USING btree (tournament_id, tournament_snapshot_id, n_sims, seed, probability_source_key) WHERE (deleted_at IS NULL);
...
CREATE INDEX idx_lab_pipeline_calcutta_runs_evaluation_job ON lab.pipeline_calcutta_runs USING btree (evaluation_job_id) WHERE (evaluation_job_id IS NOT NULL);
CREATE INDEX idx_lab_pipeline_calcutta_runs_optimization_job ON lab.pipeline_calcutta_runs USING btree (optimization_job_id) WHERE (optimization_job_id IS NOT NULL);
CREATE INDEX idx_lab_pipeline_calcutta_runs_pipeline_run_id ON lab.pipeline_calcutta_runs USING btree (pipeline_run_id);
CREATE INDEX idx_lab_pipeline_calcutta_runs_predictions_job ON lab.pipeline_calcutta_runs USING btree (predictions_job_id) WHERE (predictions_job_id IS NOT NULL);
...
CREATE TRIGGER trg_core_role_permissions_updated_at BEFORE UPDATE ON core.role_permissions FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();
...
CREATE TRIGGER trg_core_tournament_imports_updated_at BEFORE UPDATE ON core.tournament_imports FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();
...
CREATE TRIGGER trg_idempotency_keys_set_updated_at BEFORE UPDATE ON core.idempotency_keys FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();
...
CREATE TRIGGER trg_derived_tournament_snapshot_teams_updated_at BEFORE UPDATE ON derived.tournament_snapshot_teams FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();
CREATE TRIGGER trg_derived_tournament_snapshots_updated_at BEFORE UPDATE ON derived.tournament_snapshots FOR EACH ROW EXECUTE FUNCTION core.set_updated_at();
...
ALTER TABLE ONLY core.grants ADD CONSTRAINT grants_granted_by_fkey FOREIGN KEY (granted_by) REFERENCES core.users(id);
...
ALTER TABLE ONLY core.grants ADD CONSTRAINT grants_role_id_fkey FOREIGN KEY (role_id) REFERENCES core.roles(id);
...
ALTER TABLE ONLY core.role_permissions ADD CONSTRAINT role_permissions_permission_id_fkey FOREIGN KEY (permission_id) REFERENCES core.permissions(id);
ALTER TABLE ONLY core.role_permissions ADD CONSTRAINT role_permissions_role_id_fkey FOREIGN KEY (role_id) REFERENCES core.roles(id);
...
ALTER TABLE ONLY core.users ADD CONSTRAINT users_invited_by_fkey FOREIGN KEY (invited_by) REFERENCES core.users(id);
...
ALTER TABLE ONLY derived.simulated_tournaments ADD CONSTRAINT tournament_simulation_batches_tournament_state_snapshot_id_fkey FOREIGN KEY (tournament_snapshot_id) REFERENCES derived.tournament_snapshots(id);
ALTER TABLE ONLY derived.tournament_snapshot_teams ADD CONSTRAINT tournament_state_snapshot_tea_tournament_state_snapshot_id_fkey FOREIGN KEY (tournament_snapshot_id) REFERENCES derived.tournament_snapshots(id) ON DELETE CASCADE;
ALTER TABLE ONLY derived.tournament_snapshot_teams ADD CONSTRAINT tournament_state_snapshot_teams_team_id_fkey FOREIGN KEY (team_id) REFERENCES core.teams(id);
ALTER TABLE ONLY derived.tournament_snapshots ADD CONSTRAINT tournament_state_snapshots_tournament_id_fkey FOREIGN KEY (tournament_id) REFERENCES core.tournaments(id);
