-- Fix portfolio_teams view: remove eliminated gate from actual_points.
-- The tournament champion (Florida) has eliminated = false because they never lose,
-- so gating actual_points on eliminated = true yields 0 for the winner.
    ...
    SELECT eb.entry_id,
        ...
        (
            CASE
                ...
                ELSE (0)::double precision
            END *
            (core.calcutta_points_for_progress(eb.calcutta_id, eb.wins, eb.byes))::double precision
        ) AS actual_points,
        (
            CASE
                WHEN (eb.team_total_bid_points > (0)::double precision) THEN (eb.bid_points / eb.team_total_bid_points)
