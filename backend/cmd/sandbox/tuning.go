...
func seedMeanStdFromTrainReturnsRows(rows []trainReturnsRow) seedStats {
...
func chooseShrinkAlphaSegmentsLOOCVRidgeReturns(train []trainReturnsRow, candidateAlphas []float64) (float64, float64, float64) {
	if len(candidateAlphas) == 0 {
		candidateAlphas = []float64{0, 0.25, 0.5, 0.75, 1.0}
	}
	...
	for _, holdoutID := range foldIDs {
		...
		model, ok := fitRidgeReturnsBidShareModel(trainFold)
		if !ok {
			continue
		}
		stats := seedMeanStdFromTrainReturnsRows(trainFold)
			...
			for _, aTop8 := range candidateAlphas {
				for _, aOther := range candidateAlphas {
					...
					for i, r := range holdoutRows {
						seg := seedSegmentIdx(r.Seed)
						alpha := combo[seg]
						predResid := ridgeReturnsPredict(model.Beta, model.SeedNetRtgMean, model.SeedNetRtgStd, r.Seed, nullableFloatToPtr(r.KenPomNetRtg), r.ExpPointsShare, r.ROIFeature)
						pred := r.SeedBaseline + alpha*predResid
						pred = clamp01(pred)
						predByRow[i] = pred
						a := predAggBySeed[r.Seed]
						a.Sum += pred
