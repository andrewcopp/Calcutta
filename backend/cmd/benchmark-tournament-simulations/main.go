...
func main() {
	...
	flag.StringVar(&probabilitySourceKey, "probability-source-key", "go_benchmark", "Stored in analytics.tournament_simulation_batches.probability_source_key")
	flag.Parse()
...
func resolveTournamentIDs(ctx context.Context, pool *pgxpool.Pool, season int) (*tournamentIDs, error) {
	...
	if coreID == nil || *coreID == "" {
		return nil, fmt.Errorf("lab_bronze.tournaments.core_tournament_id is NULL for season=%d", season)
	}
