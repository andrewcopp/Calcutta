...
def generate_tournament_value_from_simulations(
    *,
    simulated_tournaments: pd.DataFrame,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    Generate tournament value artifacts from simulated tournament outcomes.
    
    This is an alternative to generate_tournament_value that works directly
    from simulation results rather than predicted game outcomes.
    
    Returns:
        (production_df, debug_df)
    """
    required = ["sim_id", "team_key", "wins"]
    missing = [c for c in required if c not in simulated_tournaments.columns]
    if missing:
        raise ValueError(
            "simulated_tournaments missing columns: " + ", ".join(missing)
        )

    df = simulated_tournaments.copy()
    df["wins"] = pd.to_numeric(df["wins"], errors="coerce").fillna(0).astype(int)

    # Calculate points for each simulation
    df["points"] = df["wins"].apply(lambda w: points.team_points_fixed(int(w + 1)))

    # Calculate statistics by team
    stats = df.groupby("team_key").agg({
        "points": ["mean", "std", "var"],
        "wins": ["mean", "max"],
        "sim_id": "count"
    }).reset_index()
    ...
    # Calculate round probabilities
    round_probs = []
    for round_num in range(1, 8):
        # Round 1 = 0 wins, Round 2 = 1 win, ..., Round 7 = 6 wins
        wins_needed = round_num - 1
        prob = df[df["wins"] >= wins_needed].groupby("team_key").size() / df.groupby("team_key").size()
        prob = prob.reset_index()
        prob.columns = ["team_key", f"p_round_{round_num}"]
        round_probs.append(prob)

    # Merge all round probabilities
    debug = stats.copy()
    for prob_df in round_probs:
        debug = debug.merge(prob_df, on="team_key", how="left")
    ...
    production = debug[["team_key", "expected_points_per_entry"]].copy()
    ...
    debug_cols += [f"p_round_{r}" for r in range(1, 8)]
    debug_cols += ["avg_wins", "max_wins", "num_sims"]
    debug = debug[debug_cols].copy()
